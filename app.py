...
def proxy_get(url: str, headers: dict = None, params: dict = None) -> requests.Response:
    ...
    # 1. Determine service for reporting
...
def fetch_ncbi():
    ...
    Query params:
      ...
      - format  : 'fasta' | 'genbank'  (default: fasta)
                  'genbank' returns the full annotated GenBank / GenPept record
                  including FEATURES table (CDS, exon, gene, etc.)

    Returns:
      text/plain — raw FASTA or GenBank/GenPept record
      application/json — on error, with scientific context
    """
...
def ensembl_translate():
    """
    Fetches the CDS for an Ensembl gene or transcript ID and performs
    server-side translation using Biopython (standard codon table 1).
...
def gene_map():
    ...
    if is_acc:
        ...
        try:
            ...
            if uids:
                ...
                if NCBI_API_KEY: link_url += f"&api_key={NCBI_API_KEY}"
                enforce_rate_limit("ncbi")
    ...
    try:
        ...
        if search_query:
            ...
            if r_up.ok:
                ...
                if data:
                    # Prefer reviewed entries if multiple exist
                    reviewed = [x for x in data if x.get("entryType") == "UniProtKB reviewed (Swiss-Prot)"]
    ...
    # ── 4. Confidence Score ─────────────────────────────
    score = 0.0
    if out["ncbi_gene_id"]: score += 0.25
    if out["ensembl_id"]:   score += 0.25
    if out["uniprot_id"]:   score += 0.25
    if out["kegg_id"]:      score += 0.25
